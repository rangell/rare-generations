...
paths = ["/home/horvitz/red_team_from_cluster/post_ICLR_auto_red_team/NEW_RESULTS_WITH_BASE_OUTPUT_SAMPLES/*"]
...
for path in combined_paths:
    ...
    # expected_path = os.path.join(path, "eval_results_BASE_PROPOSAL_100.json")
    expected_path = os.path.join(path, "eval_results_BASE_PROPOSAL_100.json")
    # expected_path = os.path.join(path, "eval_results_BASE_PROPOSAL_1.json")
...
for setting, results in setting_to_results.items():
    ...
    for p in range(len(results[0])):
        ...
        prompt_scores = prompt_scores[0]
        # prompt_scores = float(np.max(prompt_scores) > 0.5)
