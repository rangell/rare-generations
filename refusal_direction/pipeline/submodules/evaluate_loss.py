...
def batch_iterator_chat_completions(
    ...
    while True:
        ...
        filtered_instructions_batch = []
        filtered_outputs_batch = []
        for in_str, out_str in zip(instructions_batch, outputs_batch):
            if out_str != "":
                filtered_instructions_batch.append(in_str)
                filtered_outputs_batch.append(out_str)

        if len(filtered_instructions_batch) == 0:
            break

        inputs = tokenize_instructions_fn(
            instructions=filtered_instructions_batch, outputs=filtered_outputs_batch
        )
