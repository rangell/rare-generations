...
if __name__ == "__main__":
    ...
    # score responses and save to disk
    output_fname = (
        f"{args.output_dir}/{args.target_model.split('/')[1]}-responses-test.json"
    )
    with Parallel(n_jobs=1000, prefer="threads") as parallel:
        with open(output_fname, "w") as f:
            for zip_tuple in tqdm(zip(jailbreaks_dataset, *outputs)):
                in_data = zip_tuple[0]
                meta_output = list(zip_tuple[1:])
                scoring_fn = lambda r: strongreject_rubric(
                    in_data["forbidden_prompt"], r
                )["score"]
                all_responses = [
                    r.text for output in meta_output for r in output.outputs
                ]
                response_set = list(
                    set([r.text for output in meta_output for r in output.outputs])
                )
                response_set_scores = parallel(
                    delayed(scoring_fn)(resp) for resp in response_set
                )
                response_to_score = {
                    r: s for r, s in zip(response_set, response_set_scores)
                }

                print(
                    f"forbidden prompt: {in_data['forbidden_prompt']}\n",
                    {r: s for r, s in response_to_score.items() if s > 0.0},
                )
                print(
                    f"total generations: {len(all_responses)}, unique generations: {len(response_set)}"
                )
                print("\n")
