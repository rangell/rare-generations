...
def create_model_wrapper(model, tokenizer, fwd_pre_hooks=[], fwd_hooks=[]):
    ...
    ds_config = {
        ...
        "pipeline": {
            "micro_batch_size": 32, 
            "pipeline_parallel_size": train_batch_size, #world_size
        },
        "steps_per_print": 2000,
        "train_batch_size": train_batch_size,
        "train_micro_batch_size_per_gpu": train_batch_size // world_size, #1,
    ...
    def model_forward(
        ...
        #     if "cuda" not in str(model.device):
        #         model = model.to("cuda")

        #     # ensure that the input are on the correct device
        #     model_inputs = {k: v.to(model.device) if isinstance(v, torch.Tensor) else v for k, v in model_inputs.items()}
...
def generate(
    ...
    for sample_idx in tqdm(range(max_new_tokens)):
        ...
        if use_smc:
            ...
            indices = torch.arange(num_particles)
...
#####
def get_output_and_harmfulness_estimate(
    base_forward,
    proposal_forward,
    forbidden_prompt,
    tokenizer,
    ...
    outputs, harmfulness_estimate = generate(
        ...
        smc_verbose=True,
        r_fn=r_fn,
        gen_batch_size=32, #1000,
    )

    dist.barrier()

    return outputs, harmfulness_estimate
...
if __name__ == "__main__":
    ...
    for example in mc_dataset:
        ...
        outputs, harmfulness_estimate = get_output_and_harmfulness_estimate(
            base_forward=base_forward,
            proposal_forward=proposal_forward,
            forbidden_prompt=forbidden_prompt,
            tokenizer=tokenizer,
        ...
        print(f"Harmfulness estimate: {harmfulness_estimate}")
