    ...
    for key in range(len(is_model_logs)):
        ...
        for key in is_output:
            if "_judge_scores" not in key:
                continue
            is_harm_scores_idx.extend(is_output[key])
            is_arr_idx.extend(
                is_output[key.replace("_judge_scores", "_importance_weights")]
            )
            kl_arr_idx.extend(is_output[key.replace("_judge_scores", "_prompt_kl")])
        ...
        threshold_is = {
            threshold: (is_harm_scores_idx > threshold).astype(float) * is_arr_idx for threshold in thresholds
        }
        threshold_mc = {
            threshold: (mc_harm_scores_idx > threshold).astype(float) for threshold in thresholds
        }
        ...
        model_comparison_arr['is_score_mean'].append((is_harm_scores_idx * is_arr_idx).mean())
        model_comparison_arr['threshold_is'].append(threshold_is)
        model_comparison_arr['threshold_mc'].append(threshold_mc)
        model_comparison_arr['threshold_is_means'].append({k: v.mean() for k, v in threshold_is.items()})
        model_comparison_arr['threshold_mc_means'].append({k: v.mean() for k, v in threshold_mc.items()})
        model_comparison_arr['kl_arr'].append(kl_arr_idx)

    return model_comparison_arr


def make_log_log_plots(model_comparison_arr, output_dir):
