...
if __name__ == "__main__":
    ...
    exp_command = exp_command.format(
        config.output_dir, config.model_name, config.num_particles, config.seed
    )
    ...
    config.jailbreak_dataset = (
        f"combined_paraphrased_results_with_mc_{model_shortname}.json"
    )
    assert Path(
        config.jailbreak_dataset
    ).exists(), f"Jailbreak dataset {config.jailbreak_dataset} does not exist. Please run smc/create_fake_mc.py to create the combined paraphrased results with MC estimates file before running this experiment."
    ...
    exp_command = exp_command.format(
        config.model_name, config.output_dir, config.jailbreak_dataset
    )
