...
from est_unsafe_to_unsafe import get_model_name, get_base_prompt, get_proposal_prompt
...
def get_exp_args():
    ...
    parser.add_argument(
        ...
        type=int,
        nargs="+",
        default=[10],#[200, 200, 200, 200, 200],
    ...
    if True: #combined_args.save_output:
...
if __name__ == "__main__":
    ...
    path = "malicious_uses.jsonl"
    with open(path, "r") as f:
        mc_dataset = [json.loads(line) for line in f]

    max_new_tokens = args.max_new_tokens  # 150
    model_logs = {}

    with open(os.path.join(output_dir, "model_toxic_gen.jsonl"), "w") as out_file:
        for example_idx, example in enumerate(tqdm(mc_dataset)):
            # example_model_logs = {}
            # forbidden_prompt = example["forbidden_prompt"]
            forbidden_prompt = example["question"]
            messages = [
                {
                    "role": "user",
            ...
            for num_particles_idx, num_particles in enumerate(args.num_particle_arr):
                ...
                outputs = [
                    ...
                    for r in outputs["proposal"]
                ]

                save_dict = {}
                save_dict["forbidden_prompt"] = forbidden_prompt
                save_dict["judge_scores"] = judge_scores.cpu().tolist()
                save_dict["importance_weights"] = importance_weights.cpu().tolist()
